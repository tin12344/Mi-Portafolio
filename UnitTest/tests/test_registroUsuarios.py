...
# Clase de prueba para la funcionalidad de registro de usuarios
class TestRegistroUsuarios(unittest.TestCase):
    ...
    def test_registro_exitoso_usuario(self):
        # Prueba de registro exitoso de un usuario
        self.biblioteca.registrar_usuario("Juan Perez", "u123")

        # Verificar que se hicieron las llamadas correctas al cursor
        self.cursor_mock.execute.assert_called_with(
            "INSERT INTO usuarios (id, nombre) VALUES (%s, %s)",  # Verifica la consulta de inserción del usuario
            ("u123", "Juan Perez")  # Datos del usuario a insertar
        )
        self.conn_mock.commit.assert_called_once()  # Verifica que se haya llamado a commit para guardar los cambios

    def test_registro_fallido_usuario_ya_existente(self):
        # Prueba de registro fallido debido a que el usuario ya está registrado
        self.cursor_mock.execute.side_effect = ValueError("El usuario ya está registrado")  # Configura el mock del cursor para simular una excepción de integridad
        with self.assertRaises(ValueError):  # Espera que se lance un ValueError
            self.biblioteca.registrar_usuario("Juan Perez", "u123")  # Intenta registrar un usuario ya existente
